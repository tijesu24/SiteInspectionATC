...
class SiteVisit {
  ...
  String intToOkNOtOkNA(int value) {
    return value == 0
        ? "NA"
        : value == 1
            ? "Ok"
  ...
  String textExport() {
    ...
    String export = siteId.toUpperCase() +
        '''
 visit on ''' +
        format.format(dateofVisit) +
        '''
\n\nDG Capacity : $dgCapacity KVA
Gen Run Hour : $genRH
Last PPM (RH) : $lastPPM
DG due for PPM(Yes/No) :''' +
        boolToYesNo(dgDuePPM) +
        '''
\nFrequency : $freq Hz
DG AC Voltage : $dgACVolts V
DG AC Load : $dgACLoad A
Engine Oil Level : ''' +
        boolToYesNo(engOilOk) +
        '''
\nRadiator Status : ''' +
        boolToYesNo(radOk) +
        '''
\nDiesel Level(Tank dip) cm/litres : $dieselDipcm cm/ $dieselDipLts lts
Galooli Diesel Level : $dieselGal lts
PHCN Functional (Yes/No) : ''' +
        boolToYesNo(phcnOk) +
        '''
\nHybrid Functional (Yes/No) : ''' +
        boolToYesNo(hybridOk) +
        '''
\nDC/Rectifier Load : $dcLoad A
No of Working Modules : $moduleNo
Security Light status : ''' +
        intToOkNOtOkNA(secLtOk) +
        '''
\nAviation Light status  : ''' +
        intToOkNOtOkNA(avLtOk) +
        '''
\nSite Janitorial Status : ''' +
        boolToYesNo(janitOk) +
        '''
\nComment : $comment
          ''';
